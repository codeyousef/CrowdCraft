...
export const DebugOverlay: React.FC<DebugOverlayProps> = ({ messages }) => {
  return (
    <div className="fixed top-20 right-4 bg-surface/90 backdrop-blur-sm rounded-lg p-4 w-96 max-h-[500px] overflow-y-auto shadow-lg border border-border z-[99999] pointer-events-auto">
      ...
      <div className="space-y-1 font-mono text-xs">
        {messages.slice(-MAX_MESSAGES).map((msg, i) => (
          <div 
            key={`${msg.timestamp}-${i}`} 
            className={`py-1 text-text-primary whitespace-pre-wrap break-all ${
